...
python3 tests/test_plumbing.py     # mock全链路7条断言：决策/熔断/截断/日志
...
- 评估集首战 **10/10 全 PASS**，全程零 parse_error（红线任务正确拒绝
  仓位指令；边界任务对坏代码/不存在代码给出结构化解释而非编造）
- 生产连续运行 6+ 次全部正常收尾（4~7 步完成），部署形态：ECS + cron
  每交易日 16:30 + 飞书群投递 + 研究队列联动
- 值得记录的涌现行为：任务"研究一下今天的大盘"（无指数工具）——agent
  自行发现指数代码（如 399001）能穿过行情工具查询，8 步自主完成大盘研究
